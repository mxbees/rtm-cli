...
tasks2tsv () {
  ...
  all_tasks=$(mktemp)
  ./json.sh < data/39537783.json | tail -n +2 > "$all_tasks"
...
task_loop () {
  ...
  while read -r task_list task_series_id task_id priority name due created modified; do
    ...
    list_name=$(grep "$task_list" $rtm_lists | cut -f2)
    if [[ "$due" =~ $r ]]; then
...
#then by due date.
_by_priority () {
  sort -k 4 $tasks > data/pri.tsv
}
#you can get it sorted by date if you prefer.
_by_due_date () {
  sort -k 6 $tasks > data/due.tsv
}
